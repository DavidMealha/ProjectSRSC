...
public class MChatCliente extends JFrame implements MulticastChatEventListener {
	...
	// Construtor para uma frame com do chat multicast (inicializado em estado
	// nao conectado)
	public MChatCliente() {
		...
		JScrollPane textAreaScrollPane = new JScrollPane(textArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
		getContentPane().add(textAreaScrollPane, BorderLayout.CENTER);

		users = new DefaultListModel();
		JList usersList = new JList(users);
		JScrollPane usersListScrollPane = new JScrollPane(usersList, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER) {
			@Override
	...
	/**
	 * Inicializa lista de utilizadores a partir de um iterador -- pode ser
	 * usado obtendo iterador de qualquer estrutura de dados de java
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Envia mensagem. Chamado quando se carrega no botao de SEND ou se faz
	 * ENTER na linha da mensagem. Executa operacoes relacionadas com interface
	 * -- nao modificar
	 */
	...
	protected void doSendMessage(String message) {
		try {
			...
			JOptionPane.showMessageDialog(this, "Erro ao enviar uma menssagem: " + ex.getMessage(), "Chat Error",
	...
	/**
	 * Pede downlaod dum ficheiro. Chamado quando se carrega no botao de SEND ou
	 * se faz ENTER na linha de download. Executa operacoes relacionadas com
	 * interface -- nao modificar
	 */
	 ...
	 * Executa operacoes relativas ao envio de mensagens.
	 * 
	 * NOTA: Qualquer informacao ao utilizador deve ser efectuada usando o
	 * metodo "displayMsg".
	 */
	...
	protected void onQuit() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Erro no termino do chat:  " + ex.getMessage(), "ERRO no Chat",
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public static void main(String[] args) {
		if ((args.length != 3) && (args.length != 4)) {
			System.err.println("Utilizar: MChatCliente " + "<nickusername> <grupo IPMulticast> <porto> { <ttl> }");
